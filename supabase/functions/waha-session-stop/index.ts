...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Erro ao parar sessão WAHA:', errorText);
      return new Response(
        JSON.stringify({ error: `Erro ao parar sessão: ${response.statusText}` }),
    ...
    console.error('Error in waha-session-stop:', error);
