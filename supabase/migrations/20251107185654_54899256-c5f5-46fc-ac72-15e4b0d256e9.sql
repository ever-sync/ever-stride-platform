...
-- Drop existente se houver
DROP TABLE IF EXISTS waha_sessions CASCADE;
...
CREATE TABLE waha_sessions (
    ...
    status VARCHAR(50) DEFAULT 'disconnected' CHECK (status IN ('disconnected', 'qr_code', 'connecting', 'connected', 'failed')),
...
CREATE TABLE token_usage (
    ...
    custo_total_usd DECIMAL(10,6) GENERATED ALWAYS AS (custo_input_usd + custo_output_usd + custo_cache_usd) STORED,
...
DROP TABLE IF EXISTS limites_tokens CASCADE;

CREATE TABLE limites_tokens (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE POLICY "Tenants can manage limits"
    ...
    USING (
        client_id IN (SELECT id FROM whatsapp_clients WHERE tenant_id = get_user_tenant_id(auth.uid()))
